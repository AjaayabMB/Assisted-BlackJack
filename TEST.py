...
for i in range(0, 3):
    ...
    if split_score(i, 0) != 0:
        for j in range(0, c):
            ...
            sp_hit = input("Would you like to hit or stand on your split hand?(Hit=H, Stand=S,Split=SP,Double=D)")
            match sp_hit.upper():
                case "H":
                ...
                case "D":
                    ...
                    print("Your split hand no.", j + 1, "is", splits[i][j], "And score is", split_score(i, j))
                    break
        sp_hit = input("Would you like to hit again or stand?(Hit=H,Stand=S)")
